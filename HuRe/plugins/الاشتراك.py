...
# <https://www.github.com/Source-Ze/ZESOURCE/tree/HuRe/LICENSE/>.
import asyncio
import requests
import logging
...
async def supc(event):  # Source-Ze - ZESOURCE
    ...
    ar_dev = (5650717789, 9259, 517, 5280)
    ...
    if gvarstatus ("sub_private"):
        try:
            ...
            try:
                ...
                return await l313l.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ \n{r}**")
            ...
            if "not found" in reqt:
                try:
                    ...
                    await l313l.tgbot.send_message(BOTLOG_CHATID, f"- خطـأ \n{er}")
            if "left" in reqt:
                try:
                    c = await l313l.get_entity(ch)
                    chn = c.username
            ...
            if "error_code" in reqt:
                await l313l.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ غير معروف قم باعادة توجيه الرسالة ل@UP_UO لحل المشكلة\n{reqt}**")
