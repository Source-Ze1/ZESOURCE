...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**۞︙ قام مطورين السورس بتحديث  زد إي**\n۞︙ **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ۞︙ تم تحديث سورس  زد إي بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**۞︙ يـتـم البـحـث عـن تـحديثـات سـورس  زد إي انـتـظـر**")
    ...
    if conf == "الان":
        await event.edit("** ۞︙ جار تحـديـث سـورس  زد إي انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "**۞︙ جارِ تحديث ريبو التنصيب لسورس  زد إي **")
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("**۞︙ جارِ اعادة تنصيب سورس  زد إي, انتظر قليلاً ..**")
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "الان":
            await event.edit("** ۞︙ يتم تحديث سورس  زد إي بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
